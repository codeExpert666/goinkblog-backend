...
// @Param author_id query uint false "作者ID" minimum(1)
// @Param status query string false "状态" Enums(published, draft)
// @Param sort_by query string false "排序方式" Enums(newest, popular, most_commented, most_liked) default(newest)
...
func (h *ArticleHandler) GetArticle(c *gin.Context) {
	...
	data, err := h.ArticleService.GetArticleByID(ctx, uint(id))
	if err != nil {
		util.ResError(c, err)
		return
	}
	...
	if userID > 0 {
		interactions, err := h.ArticleService.GetArticleInteractions(ctx, userID, uint(id))
		if err != nil {
			logging.Context(ctx).Error("获取用户与文章的交互状态失败", zap.Uint("article_id", uint(id)), zap.Uint("user_id", userID), zap.Error(err))
		} else {
...
func (h *ArticleHandler) UploadCover(c *gin.Context) {
	coverURL, err := h.ArticleService.UploadCover(c)
	if err != nil {
		util.ResError(c, err)
		return
	}
...
// @Summary 点赞/取消点赞文章
// @Param id path uint true "文章ID"
// @Success 200 {object} util.ResponseResult{data=map[string]bool{liked=bool}}
// @Failure 400 {object} util.ResponseResult
// @Failure 500 {object} util.ResponseResult
...
func (h *ArticleHandler) LikeArticle(c *gin.Context) {
	...
	ctx := c.Request.Context()
	userID := util.FromUserID(ctx)
	result, err := h.ArticleService.LikeArticle(ctx, userID, uint(id))
	if err != nil {
		util.ResError(c, err)
		return
	}
...
// @Summary 收藏/取消收藏文章
// @Param id path uint true "文章ID"
// @Success 200 {object} util.ResponseResult{data=map[string]bool{favorited=bool}}
// @Failure 400 {object} util.ResponseResult
// @Failure 500 {object} util.ResponseResult
...
func (h *ArticleHandler) FavoriteArticle(c *gin.Context) {
	...
	ctx := c.Request.Context()
	userID := util.FromUserID(ctx)
	result, err := h.ArticleService.FavoriteArticle(ctx, userID, uint(id))
	if err != nil {
		util.ResError(c, err)
		return
	}
...
func (h *ArticleHandler) GetLatestArticles(c *gin.Context) {
	...
	ctx := c.Request.Context()
	result, err := h.ArticleService.GetLatestArticles(ctx, limit)
	if err != nil {
		util.ResError(c, err)
		return
	}
