...
type Middleware struct {
	...
	RateLimiter struct {
		Enable                  bool     `json:"enable"`
		SkippedPathPrefixes     []string `json:"skipped_path_prefixes"`
		MaxRequestPerIPMinute   int      `json:"max_request_per_ip_minute"`
		MaxRequestPerUserSecond int      `json:"max_request_per_user_second"`
		MaxAIRequestPerUserHour int      `json:"max_ai_request_per_user_hour"`
		Redis                   struct {
			Addr     string `json:"addr"`
			DB       int    `json:"db"`
			Username string `json:"username"`
